...
def run(graph, save_on_github=False, main_entity=None):
    ...
    if type(main_entity) == OntoClass:
        ontotemplate = open(ONTODOCS_VIZ_TEMPLATES + "splitter/splitter_classinfo.html", "r")
        context.update({ "main_entity" : main_entity,
                         "main_entity_type": "class"
                         })
    elif type(main_entity) == OntoProperty:
        ontotemplate = open(ONTODOCS_VIZ_TEMPLATES + "splitter/splitter_propinfo.html", "r")
        context.update({ "main_entity" : main_entity,
                         "main_entity_type": "property"
                         })
    elif type(main_entity) == OntoSKOSConcept:
        ontotemplate = open(ONTODOCS_VIZ_TEMPLATES + "splitter/splitter_conceptinfo.html", "r")
        context.update({ "main_entity" : main_entity,
                         "main_entity_type": "concept"
                         })
    else:
            # if type(main_entity) == Ontology:
        ontotemplate = open(ONTODOCS_VIZ_TEMPLATES + "splitter/splitter_ontoinfo.html", "r")
...
if __name__ == '__main__':
    ...
    try:
        ...
        entities = [g.classes, g.all_properties, g.all_skos_concepts]
